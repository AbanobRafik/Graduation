...
router.get("/titles/:grade/:subject", (req, res) => {
    ...
    try {
        const dataPath = path_1.default.join(__dirname, `../data/firstTerm/${grade}/${subject}.json`);
        const lessons = JSON.parse(fs_1.default.readFileSync(dataPath, "utf-8"));
        const titles = lessons.map((lesson) => ({
            id: lesson.id,
            title: lesson.title,
        }));
        // إضافة رؤوس لتفادي التخزين المؤقت
...
router.get("/:grade/:subject/:id", (req, res) => {
    ...
    try {
        ...
        if (lesson) {
            ...
            res.status(404).json({ message: "الدرس غير موجود في الترم الأول" });
        }
    }
    catch (error) {
...
// 3️⃣ مسار لجلب نموذج 3D معين بناءً على المادة والصف
router.get("/model/:grade/:subject/:id", (req, res) => {
    ...
    try {
        ...
        if (lesson) {
            ...
            res.status(404).json({ message: "النموذج غير موجود في الترم الأول" });
        }
    }
    catch (error) {
